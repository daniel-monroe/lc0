...
namespace {
  bool MateDefend(const Position& pos, int depth);

  Move MateFind(const Position& pos, int depth) {
    if (depth <= 0) return Move();
    const auto& board = pos.GetBoard();
    std::vector<Move> legal_moves = board.GenerateLegalMoves();

    for (const auto& move : legal_moves) {
      const Position child(pos, move);
      if (MateDefend(child, depth - 1)) return move;
    }
    return Move();
  }
  ...
  bool MateDefend(const Position& pos, int depth) {
    ...
    const auto& board = pos.GetBoard();
    if (!board.IsUnderCheck()) return false; // prune moves that don't give check
    std::vector<Move> legal_moves = board.GenerateLegalMoves();
    ...
    for (const auto& move : legal_moves) {
      const Position child(pos, move);
      if (!MateFind(child, depth - 1)) return false; // we only need one move to defend
    }
    return true;
  }
  ...
  inline float ComputeExploreFactor(const SearchParams& params, float weight,
    ...
    const float base_factor =
        ComputeExploreFactor(params, weight, is_root_node);
...
std::vector<std::string> Search::GetVerboseStats(Node* node) const {
  ...
  auto print_stats = [&](auto* oss, const auto* n) {
    ...
    if (n) {
      ...
      *oss << "(M: " << n->GetMatingMove().as_string()
           << ") ";
    } else {
      *oss << "(WL:  -.-----) (D: -.---) (M:  -.-) ";
    }
      ...
      while (cur_limit > 0) {
        ...
        for (int idx = 0; idx < max_needed; ++idx) {
          ...
          if (idx > cache_filled_idx) {
            ...
            if (cur_iters[idx].GetMove() == node->GetMatingMove()) {
              p = 10.0;
            }
          ...
          if (can_exit) break;
          if (weightstarted == 0 && !node->GetMatingMove()) {
...
void SearchWorker::ExtendNode(NodeToProcess& picked_node) {
  ...
  if (tt_low_node != nullptr) {
    ...
    picked_node.mating_move =
        MateFind(history.Last(), params_.GetMateFinderDepth());
...
void SearchWorker::FetchSingleNodeResult(NodeToProcess* node_to_process,
  ...
  if (!node_to_process->is_tt_hit) {
    if (node_to_process->is_twin_hit) {
      ...
      if (is_tt_miss) {
        ...
        if (node_to_process->mating_move) {
          nn_eval->q = 1.0;
          nn_eval->d = 0.0;
        }
  ...
  node->SetMatingMove(node_to_process->mating_move);
